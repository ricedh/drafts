...
A few useful tips for using Voyant: To apply stop words, click on the wrench icon in the upper right-hand corner to choose a pre-made list, or add your own. Stop words are common "filler" words such as articles, prepositions, pronouns, and conjunctions. Filtering out stop words before analyzing a text will remove frequent but uninteresting words such as "the". To save a url to the current Voyant skin, create an HTML link to embed, or download an image, click on the floppy icon.
