...
These first three maps show the results using only direct hits, i.e. locations that include a state name or state abbreviation in the word. The next three maps show the result using the CountStateReferences algorithm. Note how the numbers are larger. This is because the algorithm tries to resolve detected location terms that do not include the state name in them by querying the Google Maps API for an address associated with the location. If the location is in the nearby region (the South), we assume it is an accurate result and update the state counts accordingly.
